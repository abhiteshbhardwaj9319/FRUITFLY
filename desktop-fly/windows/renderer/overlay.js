...
import { Fly, FLY_SCALE, SHADOWS_ENABLED } from '../src/flymodel.js';
...
let sim = null;
let spikeBus = null;
let signalBuilder = new SignalBuilder();
...
// Connected recording mode: this renderer remains the sole owner of the brain
// and body. The dashboard only sends electrodes and receives observations.
let recording = false;
let recordingMode = 'free';
let recordingModeAge = 0;
let recordingStimIn = 0;
let recordingTime = 0;
let recordingDistance = 0;
let recordingForwardSpeed = 0;
let recordingYawRate = 0;
let recordingLastGF = -Infinity;
let recordingGFEvents = 0;
let telemetryIn = 0;
...
const trailLine = new THREE.Line(trailGeometry, new THREE.LineBasicMaterial({
  color: 0xe6b071, transparent: true, opacity: 0.72,
}));
const trailDots = new THREE.Points(trailGeometry, new THREE.PointsMaterial({
  color: 0xf0b66d, size: 3.6, sizeAttenuation: false, transparent: true,
...
function setRecordingMode(mode) {
  const valid = ['walk', 'left', 'right', 'reverse', 'loom', 'groom', 'wings', 'tap', 'free'];
  if (!valid.includes(mode)) return;
  recordingMode = mode;
  recordingModeAge = 0;
  recordingStimIn = 0;
  if (sim) { sim.pendingStims.length = 0; sim.activeStims.length = 0; }
}

function driveRecordingMode(dt) {
  if (!recording || !sim) return;
  recordingModeAge += dt;
  recordingStimIn -= dt;
  if (recordingMode === 'loom' && recordingModeAge < 0.65) loomOverride = Math.max(loomOverride, 0.9);
  if (recordingStimIn > 0 || ['loom', 'free'].includes(recordingMode)) return;
  if (['walk', 'left', 'right'].includes(recordingMode)) sim.stimulate(sim.fwd, 0.15, 160);
  if (recordingMode === 'left') sim.stimulate(sim.dnaL, 0.3, 160);
  if (recordingMode === 'right') sim.stimulate(sim.dnaR, 0.3, 160);
  if (recordingMode === 'reverse') sim.stimulate(sim.mdn, 0.3, 160);
  if (recordingMode === 'groom') sim.stimulate(sim.groom, 0.25, 160);
  if (recordingMode === 'wings') sim.stimulate(sim.escw, 0.3, 160);
  if (recordingMode === 'tap') sim.stimulate(sim.sens, 0.45, 160);
  recordingStimIn = 0.14;
}

function resetRecording() {
  if (!loadedData) return;
  for (const fly of flies) scene.remove(fly.node);
  flies.length = 0;
  spikeBus = new SpikeBus();
  sim = new LIFSim(loadedData.circuit, spikeBus, loadedData.locomotor);
  signalBuilder = new SignalBuilder();
  simulationClock.reset(); msAccumulator = 0;
  recordingTime = 0; recordingDistance = 0; recordingForwardSpeed = 0; recordingYawRate = 0;
  recordingLastGF = -Infinity; recordingGFEvents = 0; telemetryIn = 0;
  paused = false; lastTime = null;
  trailPoints.length = 0; trailGeometry.setDrawRange(0, 0);
  addFly();
  setRecordingMode('walk');
}

function addTrailPoint(fly) {
  if (!trailEnabled || !fly) return;
  const previous = trailPoints.at(-1);
  if (previous && Math.hypot(previous.x - fly.pos.x, previous.y - fly.pos.y) < 0.5) return;
  trailPoints.push({ x: fly.pos.x, y: fly.pos.y, z: Math.max(0.25, fly.node.position.z * 0.12) });
  if (trailPoints.length > 1200) trailPoints.shift();
  trailPoints.forEach((point, i) => {
    trailPositions[i * 3] = point.x; trailPositions[i * 3 + 1] = point.y; trailPositions[i * 3 + 2] = point.z;
  });
  trailGeometry.attributes.position.needsUpdate = true;
  trailGeometry.setDrawRange(0, trailPoints.length);
}

function sendRecordingTelemetry(fly, signals) {
  if (!recording || !sim || !fly) return;
  const legPose = fly.model.legs.map((leg) => ({ angle: leg.angle, lift: leg.lift, knee: leg.kneeAngle }));
  const wingPose = fly.model.foldedWings.children.map((wing) => ({
    x: wing.rotation.x, y: wing.rotation.y, z: wing.rotation.z,
  }));
  const snapshot = {
    time: recordingTime, paused, mode: recordingMode,
    rates: [sim.rateFwd, sim.rateDNaL, sim.rateDNaR, sim.rateMDN, sim.rateLoom],
    gfEvents: recordingGFEvents, lastGF: recordingLastGF,
    forwardSpeed: recordingForwardSpeed, yawRate: recordingYawRate,
    distance: recordingDistance,
    motorCommands: sim.locomotor?.commands.map((command) => ({ ...command })) || [],
    contacts: fly.legFeedback.map((feedback) => feedback.contact),
    fly: {
      pos: { ...fly.pos }, heading: fly.heading, pitch: fly.pitch, state: fly.state,
      z: fly.node.position.z, scale: fly.node.scale.x, legs: legPose, wings: wingPose,
      blurVisible: fly.model.blurWingL.visible,
      blurOpacity: fly.model.blurWingL.material.opacity,
      blurLeftZ: fly.model.blurWingL.rotation.z, blurRightZ: fly.model.blurWingR.rotation.z,
      abdomenScaleZ: fly.model.abdomen.scale.z,
    },
    trail: trailPoints.slice(-600),
    signals: signals ? { escape: signals.escape, walkDrive: signals.walkDrive,
      backward: signals.backward, groomDrive: signals.groomDrive, wingDrive: signals.wingDrive } : null,
  };
  window.__fruitflyRecordStatus = { mode: recordingMode, time: recordingTime,
    trailEnabled, trailPoints: trailPoints.length, position: { ...fly.pos } };
  api.sendTelemetry(snapshot);
}
...
function tick(dt) {
  let signals = null;
  const first = flies[0];
  const previous = first ? { pos: { ...first.pos }, heading: first.heading } : null;
  if (sim && first) {
    driveRecordingMode(dt);
    ...
    if (recording && signals.escape) {
      if (recordingTime - recordingLastGF > 0.3) recordingGFEvents++;
      recordingLastGF = recordingTime;
    }
  ...
  if (recording && first && previous) {
    // The ordinary desktop pet is intentionally tiny. Recording mode enlarges
    // only its rendering; the body mechanics and neural feedback stay unchanged.
    const recordingScale = FLY_SCALE * 1.8 * (1 + 0.8 * first.alt);
    first.node.scale.setScalar(recordingScale);
    const dx = first.pos.x - previous.pos.x, dy = first.pos.y - previous.pos.y;
    const grounded = first.state !== 'flying';
    const k = 1 - Math.exp(-dt * 5);
    const forward = grounded ? (dx * Math.cos(previous.heading) + dy * Math.sin(previous.heading)) / dt : 0;
    const yaw = Math.atan2(Math.sin(first.heading - previous.heading), Math.cos(first.heading - previous.heading));
    recordingForwardSpeed += (forward - recordingForwardSpeed) * k;
    recordingYawRate += ((grounded ? yaw / dt : 0) - recordingYawRate) * k;
    if (grounded) recordingDistance += Math.hypot(dx, dy);
    recordingTime += dt;
    addTrailPoint(first);
    telemetryIn -= dt;
    if (telemetryIn <= 0) { sendRecordingTelemetry(first, signals); telemetryIn = 0.05; }
  }
...
api.onCommand((c) => {
  switch (c.name) {
    ...
    case 'recordSetup': recording = true; trailEnabled = true; setRecordingMode('walk'); break;
    case 'recordMode': setRecordingMode(c.mode); break;
    case 'recordTrail':
      trailEnabled = !!c.value; trailLine.visible = trailEnabled; trailDots.visible = trailEnabled; break;
    case 'recordReset': resetRecording(); break;
    default: break;
  }
});
