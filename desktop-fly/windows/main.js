...
// Keep the recording window's Chromium cache independent of a running overlay.
if (DEMO || RECORD) app.setPath('userData', path.join(HERE, 'artifacts', RECORD ? 'record-profile' : 'demo-profile'));
...
function createDashboard() {
  const win = new BrowserWindow({
    width: 1440, height: 920, minWidth: 1000, minHeight: 720,
    title: RECORD ? 'FRUITFLY | Connected recording studio' : 'FRUITFLY | Live circuit lab',
    backgroundColor: '#0c1015',
    webPreferences: { preload: path.join(HERE, 'preload.mjs'),
      backgroundThrottling: false, sandbox: false },
  });
  win.setMenu(null);
  ...
  pipeConsole(win, RECORD ? 'record' : 'demo');
  win.loadFile(path.join(HERE, 'renderer', 'demo.html'));
  win.webContents.once('did-finish-load', () => {
    console.info(`[${RECORD ? 'record' : 'demo'}] FRUITFLY window loaded. F11: fullscreen.`);
  });
...
app.whenReady().then(() => {
  ...
  if (RECORD) {
    ...
    overlay.webContents.once('did-finish-load', () => {
      send(overlay, 'cmd', { name: 'recordSetup' });
    });
  } else if (brainData) brain = createBrain(screen.getPrimaryDisplay());
...
ipcMain.handle('runtime-mode', () => RECORD ? 'record' : (DEMO ? 'demo' : 'overlay'));
ipcMain.on('spikes', (_e, list) => send(brain, 'spikes', list));
ipcMain.on('stimulate', (_e, req) => send(overlay, 'stimulate', req));
ipcMain.on('telemetry', (_e, snapshot) => send(dashboard, 'telemetry', snapshot));
ipcMain.on('dashboard-command', (_e, command) => send(overlay, 'cmd', command));
